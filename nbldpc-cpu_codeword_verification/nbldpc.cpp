...
int minmax(unsigned char*** BETAmn_, unsigned char*** ALPHAmn_, unsigned char** GAMMAn_, int* iteration, int* decoded_bit)
{
	...
	FILE *fp_final_codeword = fopen("./data/codeword_sym.txt", "w");
...
/* 
			...
			for (index_c = 0, col = col_row[row][0]; index_c < row_weight_; index_c++, col = col_row[row][index_c]) {		// 1. Initialize F and B value for each row
				if (index_c == 0) {
					...
					for (a = 0; a < Q; a++) {
						...
						//if(row==0)
						//printf("F[%d][%d][0]=%d mult=%d \n", row, a, F[a][0],mul_gf(inv_gf(H_nb[row][col]), a));
	...
	for (int col = 0; col < N; col++) {	// post processing
			for (a = 0; a < Q; a++) {
				temp = 0;
				index_ = 0;
				row_ = row_col[0][col];
		...
		for (int col = 0; col < N; col++) {	// tentative decoding
			a = 0;
			for (i = 0; i < Q; i++) {
