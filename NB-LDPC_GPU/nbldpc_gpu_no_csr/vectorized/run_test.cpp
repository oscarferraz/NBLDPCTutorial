...
#if Q==4
    unsigned short	h_row_col[1][6];
    unsigned short	h_col_row[3][2];
#elif Q==8
    unsigned short	h_row_col[1][42];
    unsigned short	h_col_row[14][3];
	#elif Q==16
    unsigned short	h_row_col[3][300];
    unsigned short	h_col_row[150][6];
#elif Q==32
    unsigned short	h_row_col[3][620];
    unsigned short	h_col_row[310][6];
#endif
...
int main(int argc, char*argv[])
{
	...
	if(Q==256){
		...
		h_alpha=(unsigned char *)malloc(sizeof(unsigned char)*Q*N*M);
		if(h_alpha == NULL){
		...
		h_beta=(unsigned char *)malloc(sizeof(unsigned char)*M*N*Q);
		if(h_beta == NULL){
			printf("Failed to allocate host gamma\n" );
			exit(EXIT_FAILURE);
		}
		...
		error = cuda_minmax(h_beta, h_alpha, h_gamma, h_F, h_B, &iteration, decoded_bit);
		...
		for (int row = 0; row < M; row++){
			//int col = col_row[row][0];
			for (int index = 0; index < row_weight[row]; index++){
				int col = col_row[row][index];
				for (int a = 0; a < Q; a++) {
					//printf("Alpha[%d][%d][%d]=%d\n", row, col,a,ALPHAmn_[row][col][a]);
					fprintf(fp_alphal_val, "%d ", h_alpha[(Q*N*row)+(Q*col)+a]);
				}
		...
		for (int row = 0; row < M; row++) {
			for (int index_c = 0, col = col_row[row][0]; index_c < row_weight[row]; index_c++, col = col_row[row][index_c]) {	//2. update BETA values from B and F values
				for (int a = 0; a < Q; a++) {
					fprintf(fp_beta_val,"%d ", h_beta[(row*N*Q)+(Q*col)+a]);
		...
		for(int row=0; row<M; row++){
			for (int a = 0; a < Q; a++) {
				for (int index = 0; index < row_weight[0]; index++) {
					//printf("x=%d y=%d z=%d\n", row, a, index);
					//printf("B=%d\n", h_B[(row*Q*row_weight[0])+(a*row_weight[0])+index ]);
					//printf("F=%d\n", h_F[(row*Q*row_weight[0])+(a*row_weight[0])+index ]);
					fprintf(fp_B_val,"%d ", h_B[(row*Q*row_weight[0])+(a*row_weight[0])+index ]);
					fprintf(fp_F_val,"%d ", h_F[(row*Q*row_weight[0])+(a*row_weight[0])+index ]);
				}
				fprintf(fp_B_val,"\n");
				fprintf(fp_F_val,"\n");
			}
